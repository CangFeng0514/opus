...
int main(int argc, char** argv) {
    ...
    // 3. 读取PCM数据
    std::ifstream pcm_file(argv[1], std::ios::binary | std::ios::ate); //文件输入流用于读取文件信息，二进制打开，定位到文件末尾
    size_t file_size = pcm_file.tellg(); // 利用tellg()获取读指针的位置，
    pcm_file.seekg(0); // 用seekg()来设置读取位置   这是一种读取二进制文件大小的做法
    
    const int frame_size = 320; // 20ms@16kHz (适合ARM的低延迟)
    std::vector<int16_t> pcm_frame(frame_size);  //​创建一个 std::vector<int16_t>，并预先分配 / 初始化 320 个 int16_t 元素
    size_t total_samples = 0;

    // 4. 编码循环（带静音检测）
    while (pcm_file.read(reinterpret_cast<char*>(pcm_frame.data()),  //vector的data方法 返回指向底层数组首元素的裸指针（raw pointer）  reinterpret_cast<char*>，将返回的指针类型转换为char*，因为read只接受char*
                       frame_size * sizeof(int16_t))) { //​​从文件流 pcm_file 中循环读取数据，每次读取 frame_size * sizeof(int16_t) 个字节（即 frame_size 个 int16_t 类型的采样点），\
                       并存储到 pcm_frame.data() 指向的内存缓冲区中，直到文件读取完毕。​参数，读取到哪儿，读取多少个字节 read函数，读取成功返回文件本身，被认为是true，失败或文件末尾返回false EOF，被认为是false,while执行一次\
                       读取320*2bytes

        // 静音检测（ARM优化版）
        bool is_silence = std::all_of(pcm_frame.begin(), pcm_frame.end(),
            [](int16_t s){ return abs(s) < 10; }); //当容器中有任何一个点绝对值大于10 ，返回false （容器中的点，即320个采样点，文件结尾可能读不到320个）
    ...
    // 5. 处理剩余样本（ARM兼容处理）
    size_t remaining = pcm_file.gcount() / sizeof(int16_t); //上一次read读取了多少个字节/每个样本2字节=剩余样本数
    if (remaining > 0) {
        std::vector<int16_t> last_frame(frame_size, 0); //申请新容器可能是因为不想倒数第二次留下来的数据，污染最后一帧数据
